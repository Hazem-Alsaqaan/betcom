import { Image, I18nManager, TouchableOpacity, View } from "react-native";
...
const UnitPicturesSlider = () => {
  ...
  const increaseIndexPicture = () => {
    setIndexPicture((prev) =>
      prev < specificUnit?.images?.length - 1 ? (prev += 1) : 0
    );
  };
  ...
  return (
    <View className={`w-full h-64 relative`}>
      <TouchableOpacity
        className={` absolute top-1/2 -translate-y-4 left-5 z-10`}
        onPress={() => decreaseIndexPicture()}
      >
        <AntDesign
          name={`${I18nManager.isRTL ? "rightcircle" : "leftcircle"}`}
          size={32}
      ...
      {specificUnitLoading ? (
        ...
        <Image
          source={{
            uri: `${
              Object.keys(specificUnit).length <= 0
      ...
      <TouchableOpacity
        className={` absolute top-1/2 -translate-y-4 right-5`}
        onPress={() => increaseIndexPicture()}
      >
        <AntDesign
