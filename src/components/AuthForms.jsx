...
const AuthForms = ({ placeholder, setValue, secure, value }) => {
    ...
    const handleAnimate = (stateClicked) => {
        if (stateClicked === "focus") {
            defaultValue.value = withTiming(changedValue.value, { duration: 300, easing: Easing.bounce })
        } else if (stateClicked === "blur") {
            defaultValue.value = withTiming(resetValue.value, { duration: 300, easing: Easing.bounce })
        }
    }
    return (
        <Animated.View className={`bg-whiteColor px-4 py-2 mx-3 my-1 border-[1px] border-solid border-gray300 rounded-full relative`} style={animateStyle}>
            <TextInput
